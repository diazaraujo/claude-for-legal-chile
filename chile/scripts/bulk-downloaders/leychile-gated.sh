...
for geo in ('GB','DE','ES'):
    try:
        p=json.dumps({'url':'https://www.bcn.cl/leychile/Consulta/obtxml?opt=7&idNorma=1044382','httpResponseBody':True,'geolocation':geo}).encode()
        ...
        d=json.loads(urllib.request.urlopen(r,timeout=8).read()); b=base64.b64decode(d.get('httpResponseBody','') or '')
