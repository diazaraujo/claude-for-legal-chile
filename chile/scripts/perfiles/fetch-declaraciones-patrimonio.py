...
def build_map():
    ...
    for d in arr:
        ...
        fa = d.get("FechaDeclaracion", "")
        if nom not in idx or fa > idx[nom][1]:
            idx[nom] = (d["IdDeclaracion"], fa, d.get("Cargo"))
    ...
    for k, n in pc.execute("SELECT juez_key,juez FROM juez_perfil WHERE juez IS NOT NULL AND n_causas>=1"):
        if norm(n) in idx:
            mp[k] = idx[norm(n)]
    pc.close()
    print(f"# jueces con declaración pública: {len(mp)}", flush=True)
    return mp
...
def main():
    ...
    def fetch(item):
        key, (idd, fecha, cargo) = item
        try:
            r = sess.get(DETALLE + str(idd), timeout=30)
            r.raise_for_status()
            return key, idd, fecha, cargo, parse_detalle(r.text), None
        except Exception as e:
            return key, idd, fecha, cargo, None, e

    seen = 0
    with ThreadPoolExecutor(max_workers=args.workers) as ex:
        futs = [ex.submit(fetch, it) for it in items]
        for f in as_completed(futs):
            key, idd, fecha, cargo, res, exc = f.result()
            seen += 1
    ...
    out.commit()
    out.close()
    print(f"\nDECLARACIONES OK · {n_total} jueces · cargados={ok} err={err}")
